...
async def lifespan_local_tunnel(app: FastAPI):
    status, tunnel_url, tunnel_process = tunnel.create_tunnel(port=APP_PORT, subdomain=lt_subdomain)
    if status:
...
if TUNNEL == "lt":
    ...
    # With ngrok, we can configure this behaviour on their dashboard.
    @app.middleware("http")
